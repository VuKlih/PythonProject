...
class Database:
    ...
    def Search(self, id):
        ...
        searchResults = self.dbCursor.fetchall() # return a list which each element is a tuple
        return searchResults
    
    def Search2(self, id):
        self.dbCursor.execute("SELECT id,fName || ' ' || lName  AS Name, dob || '-' || mob || '-' || yob AS DOB, gender, address, phone, email, billperiodfrom_date|| '/' || billperiodfrom_month || '/' || billperiodfrom_year || ' - ' || billperiodto_date || '/' || billperiodto_month || '/' || billperiodto_year AS billperiod, wateramount, charges FROM client_info WHERE id = ?", (id, ))
    ...
    def Display(self):  
        self.calculate_watercharges()
        self.dbCursor.execute("SELECT id,fName || ' ' || lName  AS Name, dob || '-' || mob || '-' || yob AS DOB, gender, address, phone, email, billperiodfrom_date|| '/' || billperiodfrom_month || '/' || billperiodfrom_year || ' - ' || billperiodto_date || '/' || billperiodto_month || '/' || billperiodto_year AS billperiod, wateramount, charges FROM client_info")
        self.dbConnection.commit()
